...
void radio_gui_build_hardware_page(radio_gui_t * gui)
{
    ...
    lv_bar_set_range(gui->battery_bar, 3000, 4200);
    ...
    gui->backlight_value = radio_gui_create_value_pill(backlight_row, "38 / 49", RADIO_GUI_COLOR_BLUE);
...
{
    if(gui == NULL || gui->amp_switch == NULL) return;
    if(enabled) lv_obj_add_state(gui->amp_switch, LV_STATE_CHECKED);
    else lv_obj_remove_state(gui->amp_switch, LV_STATE_CHECKED);
...
{
    if(gui == NULL || gui->amp_mode_dropdown == NULL) return;
...
{
    if(gui == NULL || gui->backlight_slider == NULL) return;
    if(level > 49U) level = 49U;
...
{
    if(gui == NULL || gui->amp_mode_dropdown == NULL) return RADIO_GUI_AMP_CLASS_AB;
...
{
    if(gui == NULL || gui->backlight_slider == NULL) return 0U;
...
static void amp_switch_event_cb(lv_event_t * event)
{
    ...
    if(gui->config.callbacks.audio_amp_enabled_changed != NULL) {
        gui->config.callbacks.audio_amp_enabled_changed(gui->config.callback_context, enabled);
    }
}
...
static void amp_mode_event_cb(lv_event_t * event)
{
    ...
    if(gui->config.callbacks.audio_amp_mode_changed != NULL) {
        gui->config.callbacks.audio_amp_mode_changed(gui->config.callback_context, mode);
    }
}
