...
{
    if(gui == NULL || gui->band_buttons == NULL) return;
    gui->band = band;
...
{
    if(gui == NULL || gui->frequency_value == NULL) return;
    lv_label_set_text(gui->frequency_value, text != NULL ? text : "--");
}

void radio_gui_set_volume(radio_gui_t * gui, uint8_t volume)
{
    if(gui == NULL || gui->volume_slider == NULL) return;
    if(volume > 63U) volume = 63U;
...
{
    if(gui == NULL || gui->rx_table == NULL || field >= RADIO_GUI_RX_FIELD_COUNT) return;
...
{
    if(gui == NULL || gui->rx_table == NULL || field >= RADIO_GUI_RX_FIELD_COUNT) return NULL;
