...
{
    if(gui == NULL || gui->gps_switch == NULL) return;
    if(enabled) lv_obj_add_state(gui->gps_switch, LV_STATE_CHECKED);
    else lv_obj_remove_state(gui->gps_switch, LV_STATE_CHECKED);
}

void radio_gui_set_sd_logging_enabled(radio_gui_t * gui, bool enabled)
{
    if(gui == NULL || gui->sd_log_switch == NULL) return;
    if(enabled) lv_obj_add_state(gui->sd_log_switch, LV_STATE_CHECKED);
    else lv_obj_remove_state(gui->sd_log_switch, LV_STATE_CHECKED);
}

void radio_gui_set_sd_status(radio_gui_t * gui, radio_gui_sd_status_t status)
{
    if(gui == NULL || gui->sd_status_label == NULL) return;
...
{
    if(gui == NULL || gui->gps_status_label == NULL) return;
...
{
    if(gui == NULL || gui->gps_table == NULL || field >= RADIO_GUI_GPS_FIELD_COUNT) return;
...
{
    if(gui == NULL || gui->gps_table == NULL || field >= RADIO_GUI_GPS_FIELD_COUNT) return NULL;
