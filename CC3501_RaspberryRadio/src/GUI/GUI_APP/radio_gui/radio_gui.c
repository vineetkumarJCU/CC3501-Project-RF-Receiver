...
void radio_gui_destroy(radio_gui_t *gui)
{
  if (gui == NULL)
    return;
  if (gui->keypad_overlay != NULL)
    lv_obj_delete(gui->keypad_overlay);
  if (gui->result_overlay != NULL)
    lv_obj_delete(gui->result_overlay);
  if (gui->band_confirm_overlay != NULL)
    lv_obj_delete(gui->band_confirm_overlay);
  if (gui->root != NULL)
    lv_obj_delete(gui->root);
  lv_free(gui);
}
...
void radio_gui_show_page(radio_gui_t *gui, radio_gui_page_t page, lv_anim_enable_t animated)
{
  ...
  if (page == RADIO_GUI_PAGE_HARDWARE)
    tile = gui->hardware_tile;
  else if (page == RADIO_GUI_PAGE_WAVEFORM)
    tile = gui->waveform_tile;
  else if (page == RADIO_GUI_PAGE_GPS)
    tile = gui->gps_tile;
...
{
  if (gui == NULL || gui->tileview == NULL)
    return RADIO_GUI_PAGE_RADIO;
  lv_obj_t *active_tile = lv_tileview_get_tile_active(gui->tileview);
  if (active_tile == gui->hardware_tile)
    return RADIO_GUI_PAGE_HARDWARE;
  if (active_tile == gui->waveform_tile)
    return RADIO_GUI_PAGE_WAVEFORM;
  if (active_tile == gui->gps_tile)
    return RADIO_GUI_PAGE_GPS;
  return RADIO_GUI_PAGE_RADIO;
}
...
{
  if (gui == NULL || gui->band_confirm_overlay == NULL || band == gui->band)
    return;
...
static void band_confirm_yes_event_cb(lv_event_t *event)
{
  ...
  radio_gui_set_frequency_text(gui, "NO Frequency set");
}
