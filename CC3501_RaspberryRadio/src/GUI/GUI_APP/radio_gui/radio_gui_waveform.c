                             ...
                             lv_obj_t *y_labels[3])
{
  ...
  for (uint32_t i = 0; i < x_tick_count; ++i)
  {
    ...
    lv_obj_set_style_text_align(x_labels[i],
                                i == 0U ? LV_TEXT_ALIGN_LEFT : (i + 1U == x_tick_count ? LV_TEXT_ALIGN_RIGHT : LV_TEXT_ALIGN_CENTER),
                                0);
  }
...
void radio_gui_build_waveform_page(radio_gui_t *gui)
{
  ...
  lv_obj_remove_flag(gui->time_chart, LV_OBJ_FLAG_CLICKABLE);
  lv_obj_remove_flag(gui->time_chart, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_remove_flag(gui->spectrum_chart, LV_OBJ_FLAG_CLICKABLE);
  lv_obj_remove_flag(gui->spectrum_chart, LV_OBJ_FLAG_SCROLLABLE);
}
...
void radio_gui_set_time_domain_data(radio_gui_t *gui,
                                    ...
                                    uint32_t sample_rate_hz)
{
  if (gui == NULL || gui->time_chart == NULL || gui->time_series == NULL || samples == NULL)
    return;
  ...
  int32_t *values = lv_chart_get_series_y_array(gui->time_chart, gui->time_series);

  for (uint32_t i = 0; i < RADIO_GUI_WAVEFORM_POINT_COUNT; i++)
  {
    values[i] = (samples[i] << 1) + (samples[i] >> 2); // scale for LVGL chart //times 2.25x
  }

  // Update the X-axis labels only if the sample rate has changed
  static uint32_t prev_sample_rate_hz=0;
  if(prev_sample_rate_hz != sample_rate_hz)
  {
    char middle_text[20];
    char end_text[20];
  ...
  prev_sample_rate_hz = sample_rate_hz;
                                 ...
                                 int8_t max_db)
{
  ...
  int32_t *values = lv_chart_get_series_y_array(gui->spectrum_chart, gui->spectrum_series);

  for (uint32_t i = 0; i < RADIO_GUI_WAVEFORM_POINT_COUNT; i++)
  {
    values[i] = (log_magnitude_db[i] + values[i]) >> 1;// IIR filter to smooth the spectrum display
  }

  // Update the X-axis labels only if the configuration has changed
  static uint64_t prev_donfig=0;
  uint64_t curr_config = ((uint64_t)max_frequency_hz << 32) | ((uint64_t)min_db << 8) | (uint64_t)max_db;

  if (prev_donfig != curr_config)
  {
