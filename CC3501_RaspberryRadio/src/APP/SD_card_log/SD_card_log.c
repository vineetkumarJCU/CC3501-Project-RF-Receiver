...
static uint8_t sd_card_log_days_in_month(uint16_t year, uint8_t month)
{
  ...
  if(month == 2U && sd_card_log_is_leap_year(year)) return 29U;
...
/** FatFs timestamp callback. FAT stores local time in two-second resolution. */
...
static bool sd_card_log_parse_file_index(const char *name, uint32_t *index)
{
  ...
  if(!has_digit || strcmp(cursor, ".txt") != 0) return false;
...
static FRESULT sd_card_log_open_unique_file(void)
{
  ...
  if(result != FR_OK) return result;
  if(close_result != FR_OK) return close_result;
  if(maximum_index == UINT32_MAX - 1U) return FR_DENIED;
  ...
  for(uint32_t index = maximum_index + 1U; index < UINT32_MAX; index++) {
    int length = snprintf(log_file_path,
                          ...
                          (unsigned long)index);
    if(length < 0 || (size_t)length >= sizeof(log_file_path)) return FR_INVALID_NAME;

    result = f_open(&sd_log_file, log_file_path, FA_WRITE | FA_CREATE_NEW);
    if(result == FR_EXIST) continue;
    if(result == FR_OK) log_record_number = 0U;
    return result;
  }
  return FR_DENIED;
}
...
static bool sd_card_log_append_format(size_t *used, const char *format, ...)
{
  ...
  if(used == NULL || format == NULL || *used >= sizeof(log_text_buffer)) return false;
  ...
  written = vsnprintf(log_text_buffer + *used,
                      ...
                      args);
  va_end(args);
  if(written < 0 || (size_t)written >= sizeof(log_text_buffer) - *used) return false;
  *used += (size_t)written;
  return true;
}
...
static bool sd_card_log_format_gps(size_t *used, const gps_info_t *gps_info)
{
  ...
  if(gps_info->utc_date.valid) {
    if(!sd_card_log_append_format(used,
                                  ...
                                  (unsigned)gps_info->utc_date.day)) return false;
  ...
  if(gps_info->utc_time.valid) {
    if(!sd_card_log_append_format(used,
                                  ...
                                  (unsigned)gps_info->utc_time.millisecond)) return false;
  } else if(!sd_card_log_append_format(used, "  UTC time: --\r\n")) return false;

  if(gps_info->position.valid) {
    double latitude = gps_info->position.latitude_deg < 0.0 ?
                      -gps_info->position.latitude_deg : gps_info->position.latitude_deg;
    double longitude = gps_info->position.longitude_deg < 0.0 ?
                       -gps_info->position.longitude_deg : gps_info->position.longitude_deg;
    if(!sd_card_log_append_format(used,
                                  "  Latitude: %.6f %c\r\n"
                                  "  Longitude: %.6f %c\r\n",
                                  latitude,
...
{
  const char *band = rsq_info != NULL && rsq_info->band == SD_CARD_LOG_RADIO_SW_AM_LW ?
                     "SW/AM/LW" : "FM";
  ...
  if(!sd_card_log_append_format(used,
                                ...
                                rsq_info->station_valid ? "true" : "false")) return false;
...
sd_card_log_result_t sd_card_log_start(void)
{
  ...
  if(log_file_open || filesystem_mounted) sd_card_log_release_storage();
...
{
  if(log_state != SD_CARD_LOG_STATE_LOGGING) return false;
  if(!sd_spi0_card_is_inserted()) {
...
{
  switch(state) {
  case SD_CARD_LOG_STATE_IDLE: return "idle";
  case SD_CARD_LOG_STATE_LOGGING: return "logging";
  case SD_CARD_LOG_STATE_NO_CARD: return "no card";
  case SD_CARD_LOG_STATE_ERROR: return "error";
